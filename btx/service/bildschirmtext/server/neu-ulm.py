...
def create_page(pageid, pagearg = None):
    ret = None
    # generated pages
    ...
    if not ret and pageid.startswith("7"):
        ...
        basedir = PATH_DATA + "7/"
    if not ret and pageid.startswith("3"):
        ret = Blog_UI.create_page(User.user(), pageid, pagearg)
        basedir = PATH_DATA + "3/"
...
def decode_call(s, arg1):
    if s and (s.startswith("call:") or s.startswith("error:")):
        call_split = s.split(":")
        if len(call_split) == 2:
            target = call_split[-1]
            arg2 = None
...
def confirm(inputs, input_data): 
    action = inputs.get("action")
    if action == "publish_blog":
        cept_data = bytearray(Util.create_system_message(311))
        cept_data.extend(Cept.set_cursor(24, 1))
        cept_data.extend(Cept.sequence_end_of_page())
        sys.stdout.buffer.write(cept_data)
        sys.stdout.flush()
    elif action == "edit_blog":
        if not User.user().blogging.needs_update(input_data["title"],input_data["content"],input_data["visibility"]):
            return False 
        cept_data = bytearray(Util.create_system_message(321))
        cept_data.extend(Cept.set_cursor(24, 1))
        cept_data.extend(Cept.sequence_end_of_page())
        sys.stdout.buffer.write(cept_data)
        sys.stdout.flush()
...
def system_blog_published_message():
    sys.stdout.buffer.write(Util.create_system_message(31))
    sys.stdout.flush()
    Util.wait_for_ter()

def system_blog_edit_message():
    sys.stdout.buffer.write(Util.create_system_message(32))
    sys.stdout.flush()
    Util.wait_for_ter()
...
def handle_inputs(inputs):
    ...
    if inputs.get("confirm", True):
        if confirm(inputs, input_data):
            if inputs.get("action") == "publish_blog":
                User.user().blogging.publish(input_data["title"], input_data["content"], input_data["visibility"], input_data["notes"])
                system_blog_published_message()
            elif inputs.get("action") == "edit_blog":
                User.user().blogging.update(input_data["title"], input_data["content"], input_data["visibility"])
                system_blog_edit_message()
            else:
                pass
...
while True:
    ...
    if error == 0:
        ...
        if desired_pageid == "00": # re-send CEPT data of current page
            ...
            try:
                ret = create_page(desired_pageid, desired_page_arg)
            except:
                error=10
    ...
    if desired_pageid is None:
        ...
        if "^" in links:
            ...
            try:
                (decode, arg) = decode_call(desired_pageid, None)
            except:
                (decode, arg) = ("0", None)
            if decode:
                desired_pageid = decode
                desired_page_arg = arg

        elif val_or_hash in links:
