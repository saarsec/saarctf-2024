...
def test_sanity(cls, instance, team):
    ...
    for x in instance.flag_id_types:
        assert x == 'username' or x.startswith('hex') or x.startswith('alphanum') or x == 'email' or x.startswith('pattern:') or re.match(r'^choose_k_from:\d+:.+', x) is not None, f'Invalid flag id: {x}'
